...
class WfoTableSeeder extends Seeder
{
    ...
    {
        print "Preparing to seed 'wfos' table with " . count($this->wfos) . " values..\n";
        for($i=0; $i<count($this->wfos); $i++) {
            if (count(DB::table('wfos')->where('fullname', '=', $this->wfos[$i]['fullname'])->get()) === 0) {
                DB::table('wfos')->insert($this->wfos[$i]);
                print 'Data for "' . $this->wfos[$i]['fullname'] . '" added to database.' . "\n";
            } else {
                print 'Data for "' . $this->wfos[$i]['fullname'] . '" already exists, skipping..' . "\n";
            }
        }
    }
