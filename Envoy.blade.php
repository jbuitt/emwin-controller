...
@setup
    ...
    $releases_to_keep = 5;
    $releases_dir = '/var/www/emwin-controller/releases';
    $persistent_dir = '/var/www/emwin-controller/persistent';
    $app_dir = '/var/www/emwin-controller';
    $release = date('YmdHis');
...
@task('update_symlinks', $on_servers)
    echo 'EMWIN Controller - Linking storage directory..'
    rm -rf {{ $new_release_dir }}/storage
    ln -nfs {{ $persistent_dir }}/storage {{ $new_release_dir }}/storage

    echo 'EMWIN Controller - Linking .env file..'
...
@task('db_tasks', ['on' => 'prod1'])
    ...
    echo 'EMWIN Controller - Running Library database migrations..'
    php artisan migrate --seed --force --database=library --path database/migrations/library --class=LibraryTablesSeeder

    echo 'EMWIN Controller - Creating Admin User.. (if not already created)'
...
@task('post_deploy', $on_servers)
    ...
    echo 'EMWIN Controller - Creating storage link..'
    php artisan storage:link

    echo 'EMWIN Controller - Linking current release..'
    ...
    echo 'Removing old EMWIN Controller releases..'
    ...
    echo 'EMWIN Controller - Reloading PHP process..'
